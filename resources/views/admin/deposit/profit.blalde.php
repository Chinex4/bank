        ...
        <div class="row">
            ...
            <div class="col-xl text-end">
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalCenter">
                    <span class="tf-icons bx bx-add-to-queue"></span>&nbsp; Add New Deposit
                </button>
            </div>
        </div>
                ...
                <div class="">
                    <table class="table table-hover">
                        ...
                        <tbody class="table-border-bottom-0">
                          @if (count($deposit) != 0)
                            @foreach ($deposit as $events)
                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                {{-- <td>{{ $events->user->name }}</td> --}}
                                <td>{{ $events->capital }}</td>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <a class="dropdown-item" href="javascript:void(0);">
                                              <form action="{{ route('deposit.destroy', $events->id) }}" method="POST">
                                                @method('delete')
                                                @csrf
